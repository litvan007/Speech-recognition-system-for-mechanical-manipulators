...
if __name__ == '__main__':
    ...
    sound.export_words(words_grid3, title='zero_cross_test7')

    transcription = sp_recognition(sound.words_names)
    answer = transcription.transcript()
    print(answer)

    sound.plot_voice_range(words_grid3, title='Time of VAD splitting method with zero cross rate {:f}s with {}db SNR and recognition words (test 7)'.format(all_time, SNR_db), words=answer)
